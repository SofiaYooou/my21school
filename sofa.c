...
// 	ray.start = sofa->plr.ray_dir - p / 4; // - [половина угла обзора]; // начало веера лучей
// 	ray.end = sofa->plr.ray_dir + p / 4; // + [половина угла обзора]; // край веера лучей
//   while (ray.start <= ray.end)
// 	{
// 		sofa->plr.ray_x = sofa->plr.x; // каждый раз возвращаемся в точку начала
// 		sofa->plr.ray_y = sofa->plr.y;
// 		sofa->plr.ray_x *= SCALE + 0.5;
//  		sofa->plr.ray_y *= SCALE + 0.5;
// 		while (sofa->map[(int)(sofa->plr.ray_y / SCALE)][(int)(sofa->plr.ray_x / SCALE)] != '1')
// 		{
// 			sofa->plr.ray_x += cos(ray.start);
// 			sofa->plr.ray_y += sin(ray.start);
// 			mlx_pixel_put(sofa->mlx, sofa->win, sofa->plr.ray_x, sofa->plr.ray_y, 0xCAFF33);
// 		}
